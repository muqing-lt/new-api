...
func ApiError(c *gin.Context, err error) {
	...
	if err != nil {
		...
		if masker, ok := err.(sensitiveMasker); ok {
			msg = strings.TrimSpace(masker.MaskSensitiveError())
		} else {
			msg = strings.TrimSpace(MaskSensitiveInfo(err.Error()))
		}
		if msg == "" {
			msg = "服务器内部错误"
		}
		if !DebugEnabled && shouldHideInternalError(msg) {
			SysError("api error masked: " + err.Error())
			msg = "服务器内部错误"
		}
	}
...
func shouldHideInternalError(msg string) bool {
