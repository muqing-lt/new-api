...
func getOrCreateOrderLock(tradeNo string) *orderLockEntry {
	orderLocksMu.Lock()
	defer orderLocksMu.Unlock()
	lock, ok := orderLocks[tradeNo]
	if !ok {
		lock = &orderLockEntry{}
		orderLocks[tradeNo] = lock
	}
	lock.refs++
	return lock
}
...
func EpayNotify(c *gin.Context) {
	...
	if err := model.CompleteEpayTopUp(verifyInfo.ServiceTradeNo); err != nil {
		log.Printf("易支付回调处理失败: trade_no=%s, err=%v", verifyInfo.ServiceTradeNo, err)
