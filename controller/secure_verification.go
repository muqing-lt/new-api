...
const (
	...
	// PasskeyVerificationSessionKey Passkey 验证完成时间戳
	PasskeyVerificationSessionKey = "passkey_verified_at"
...
func UniversalVerify(c *gin.Context) {
	...
	case "passkey":
		...
		// 仅在当前会话已完成 Passkey challenge/response 后放行。
		session := sessions.Default(c)
		verified = isSessionVerificationValid(session, PasskeyVerificationSessionKey, SecureVerificationTimeout)
		if !verified {
			common.ApiError(c, fmt.Errorf("请先完成 Passkey 验证流程"))
			return
		}
...
func PasskeyVerifyAndSetSession(c *gin.Context) {
	...
	now := time.Now().Unix()
	session.Set(SecureVerificationSessionKey, now)
	session.Set(PasskeyVerificationSessionKey, now)
	_ = session.Save()
}

func isSessionVerificationValid(session sessions.Session, key string, timeoutSeconds int64) bool {
	if session == nil {
		return false
	}
	verifiedAt, ok := session.Get(key).(int64)
	if !ok {
		return false
	}
	return time.Now().Unix()-verifiedAt < timeoutSeconds
}
