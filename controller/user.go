...
func GetAffCode(c *gin.Context) {
	...
	if user.AffCode == "" {
		...
		for i := 0; i < 10; i++ {
			candidate := strings.ToUpper(common.GetRandomString(6))
			updateErr := model.UpdateUserAffCode(id, candidate)
			if updateErr == nil {
				generated = candidate
				break
			}
			if updateErr.Error() == "aff_code_already_taken" {
				continue
			}
			c.JSON(http.StatusOK, gin.H{
				"success": false,
