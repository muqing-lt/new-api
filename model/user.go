...
func GetUserIdByAffCode(affCode string) (int, error) {
	...
	var user User
	err := DB.Select("id").First(&user, "UPPER(aff_code) = UPPER(?)", affCode).Error
	return user.Id, err
}

// ValidateAffCode validates affiliate code format: 4-8 chars, alphanumeric only, returns uppercase
...
func (user *User) TransferAffQuotaToQuota(quota int) error {
	...
	// 原子扣减邀请额度，避免并发下透支；兼容 MySQL/SQLite/PostgreSQL。
	res := tx.Model(&User{}).
	...
	// 获取提交后的最新 quota，用于刷新缓存。
	var latestQuota int
	if err := tx.Model(&User{}).Where("id = ?", user.Id).Select("quota").Take(&latestQuota).Error; err != nil {
		return err
	}
	...
	user.Quota = latestQuota
	gopool.Go(func() {
		if err := updateUserQuotaCache(user.Id, latestQuota); err != nil {
			common.SysLog("failed to update user quota cache after aff transfer: " + err.Error())
		}
	})
	return nil
}
...
// ParseAffCommissionTiers 解析并标准化阶梯返佣规则：
// - threshold/rate 必须为非负
// - rate 不能大于 100
// - 按 threshold 从低到高排序
// - 相同 threshold 保留更高的 rate
func ParseAffCommissionTiers(raw string) ([]AffCommissionTier, error) {
	...
	if err := common.Unmarshal([]byte(raw), &tiers); err != nil {
		return nil, fmt.Errorf("阶梯返佣规则 JSON 格式错误: %w", err)
	}

	for _, tier := range tiers {
		if tier.Threshold < 0 {
			return nil, errors.New("阶梯返佣阈值不能小于 0")
		}
		if tier.Rate < 0 || tier.Rate > 100 {
			return nil, errors.New("阶梯返佣比例必须在 0-100 之间")
		}
	}

	sort.Slice(tiers, func(i, j int) bool {
		if tiers[i].Threshold == tiers[j].Threshold {
			return tiers[i].Rate < tiers[j].Rate
		}
		return tiers[i].Threshold < tiers[j].Threshold
	})

	normalized := make([]AffCommissionTier, 0, len(tiers))
	for _, tier := range tiers {
		n := len(normalized)
		if n > 0 && normalized[n-1].Threshold == tier.Threshold {
			if tier.Rate > normalized[n-1].Rate {
				normalized[n-1].Rate = tier.Rate
			}
			continue
		}
...
func NormalizeAffCommissionTiersJSON(raw string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("阶梯返佣规则序列化失败: %w", err)
	}
...
func GetAffCommissionRate(affHistoryQuota int) int {
	...
	tiers, err := ParseAffCommissionTiers(common.AffCommissionTiers)
	if err != nil {
		return rate
	}
	// 从高到低匹配阶梯
...
func AddAffCommission(userId int, consumeQuota int) {
	if !common.AffCommissionEnabled {
		return
	}
	if consumeQuota <= 0 {
		return
	...
	// 计算佣金（使用 int64 中间计算避免溢出）
	commission := int(int64(consumeQuota) * int64(rate) / 100)
	if commission <= 0 {
		return
	}
	...
	tx := DB.Begin()
	if tx.Error != nil {
		common.SysLog(fmt.Sprintf("failed to begin aff commission transaction for inviter %d: %s", user.InviterId, tx.Error.Error()))
		return
	}
	defer func() {
		if r := recover(); r != nil {
	...
	// 更新邀请人的 AffQuota 和 AffHistoryQuota
	res := tx.Model(&User{}).Where("id = ?", user.InviterId).Updates(map[string]interface{}{
		"aff_quota":   gorm.Expr("aff_quota + ?", commission),
		"aff_history": gorm.Expr("aff_history + ?", commission),
	})
	if res.Error != nil {
		tx.Rollback()
		common.SysLog(fmt.Sprintf("failed to add aff commission for user %d: %s", user.InviterId, res.Error.Error()))
		return
	}
	if res.RowsAffected == 0 {
		tx.Rollback()
		common.SysLog(fmt.Sprintf("failed to add aff commission for user %d: inviter not found", user.InviterId))
		return
	}

	// 写入返佣明细记录（与余额变更在同一事务）
	if err = CreateAffCommissionRecord(tx, user.InviterId, userId, commission, consumeQuota, rate); err != nil {
		tx.Rollback()
		common.SysLog(fmt.Sprintf("failed to create aff commission record for inviter %d: %s", user.InviterId, err.Error()))
		return
	}
...
// generateUniqueAffCode 生成唯一的邀请码（6 字符大写，带碰撞重试）
func generateUniqueAffCode(db *gorm.DB) string {
	for i := 0; i < 5; i++ {
		code := strings.ToUpper(common.GetRandomString(6))
		var count int64
		db.Model(&User{}).Where("aff_code = ?", code).Count(&count)
		if count == 0 {
			return code
		}
	}
...
func (user *User) Insert(inviterId int) error {
	...
	user.AffCode = generateUniqueAffCode(DB)
