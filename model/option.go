...
func InitOptionMap() {
	...
	common.OptionMap["GroupMonitoringBasePrice"] = strconv.FormatFloat(common.GroupMonitoringBasePrice, 'f', -1, 64)
...
func UpdateOption(key string, value string) error {
	normalized, err := normalizeOptionBeforeSave(key, value)
	if err != nil {
		return err
	}
	value = normalized
...
func updateOptionMap(key string, value string) (err error) {
	...
	case "AffCommissionEnabled":
		...
		if parseErr != nil {
			return fmt.Errorf("AffCommissionEnabled 必须为 true 或 false")
		}
		common.AffCommissionEnabled = enabled
	case "AffCommissionRate":
		rate, parseErr := strconv.Atoi(value)
		if parseErr != nil {
			return fmt.Errorf("AffCommissionRate 必须为整数")
		}
		if rate < 0 || rate > 100 {
			return fmt.Errorf("AffCommissionRate 必须在 0-100 之间")
		}
		common.AffCommissionRate = rate
	case "AffCommissionTiers":
		normalized, normalizeErr := NormalizeAffCommissionTiersJSON(value)
		if normalizeErr != nil {
			return normalizeErr
		}
		common.AffCommissionTiers = normalized
...
func normalizeOptionBeforeSave(key string, value string) (string, error) {
	switch key {
	case "AffCommissionEnabled":
	...
	case "AffCommissionRate":
		...
		return strconv.Itoa(rate), nil
	case "AffCommissionTiers":
		return NormalizeAffCommissionTiersJSON(value)
	default:
		return value, nil
	}
}
