...
func authHelper(c *gin.Context, minRole int) {
	...
	userID, ok := interfaceToInt(id)
	if !ok {
		c.JSON(http.StatusUnauthorized, gin.H{
			"success": false,
			"message": "无权进行此操作，用户 ID 无效",
		})
		c.Abort()
		return
	}
	...
	// 以数据库中的最新角色/状态为准，避免会话中的旧权限在降权/封禁后继续生效。
	currentUser, err := model.GetUserById(userID, false)
	if err != nil || currentUser == nil {
		c.JSON(http.StatusUnauthorized, gin.H{
			"success": false,
			"message": "无权进行此操作，用户不存在或会话失效",
		})
		c.Abort()
		return
	}
...
// TokenAuthReadOnly 用于只读查询接口，仍执行完整 token 有效性校验。
// 会检查令牌状态、过期时间、额度，以及用户状态。
