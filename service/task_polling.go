...
func TaskPollingLoop() {
	for {
		...
		for platform, tasks := range platformTask {
			...
			if len(nullTasks) > 0 {
				reason := "任务缺少 upstream task id，已标记失败"
				for _, task := range nullTasks {
					failTaskAndRefund(ctx, task, reason)
				}
				logger.LogInfo(ctx, fmt.Sprintf("Fixed null task_id tasks: %d", len(nullTasks)))
			}
...
func failTaskWithCAS(ctx context.Context, task *model.Task, reason string) bool {
	if task == nil {
		return false
	}
	snap := task.Snapshot()
	...
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf("failTaskWithCAS update error for task %s: %v", task.TaskID, err))
		return false
	}
	if !won {
		logger.LogInfo(ctx, fmt.Sprintf("failTaskWithCAS: task %s already transitioned, skip", task.TaskID))
		return false
	}
	return true
}

func failTaskAndRefund(ctx context.Context, task *model.Task, reason string) {
	if !failTaskWithCAS(ctx, task, reason) {
		return
	}
	if task.Quota != 0 {
		RefundTaskQuota(ctx, task, reason)
...
func updateSunoTasks(ctx context.Context, channelId int, taskIds []string, taskM map[string]*model.Task) error {
	...
	if err != nil {
		...
		for _, upstreamID := range taskIds {
			...
			if task == nil {
				logger.LogWarn(ctx, fmt.Sprintf("missing task for upstream id %s when channel lookup failed", upstreamID))
				continue
			}
			failTaskAndRefund(ctx, task, reason)
		}
	...
	for _, responseItem := range responseItems.Data {
		...
		if task == nil {
			logger.LogWarn(ctx, fmt.Sprintf("missing task for upstream id %s", responseItem.TaskID))
			continue
		}
		...
		if responseItem.FailReason != "" || task.Status == model.TaskStatusFailure {
			...
			task.Progress = "100%"
			shouldRefund = task.Quota != 0 && snap.Status != model.TaskStatusFailure
		}
		...
		if isDone && snap.Status != task.Status {
			...
			if !won {
				logger.LogInfo(ctx, fmt.Sprintf("task %s already transitioned by another process, skip", task.TaskID))
				continue
			}
			updated = true
			...
			if !won {
				logger.LogInfo(ctx, fmt.Sprintf("task %s already updated by another process, skip", task.TaskID))
				continue
			}
			updated = true
		}
		if shouldRefund && updated {
