...
func ApplyErrorMapping(newApiErr *types.NewAPIError, errorMappingStr string) {
	...
	var cfg ErrorMappingConfig
	if err := common.Unmarshal([]byte(errorMappingStr), &cfg); err != nil {
		return
	}
	...
	for _, pattern := range cfg.Patterns {
		if matchesErrorPattern(pattern, errMessage, errCode, errType) {
			applyErrorReplacements(newApiErr, pattern)
			break
		}
	}
}

func matchesErrorPattern(pattern ErrorMappingPattern, message, code, errType string) bool {
...
func applyErrorReplacements(newApiErr *types.NewAPIError, pattern ErrorMappingPattern) {
